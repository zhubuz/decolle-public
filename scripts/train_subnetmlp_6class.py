...
## Load Data
trainingSet = IBMGestureDataset(datasetPath='/data/dvsGesture/',
                                sampleFile='/data/dvsGesture/train.txt',
                                samplingTime=1.,
                                sampleLength=500,
                                augment=True)

testingSet = IBMGestureDataset(datasetPath='/data/dvsGesture/',
                               sampleFile='/data/dvsGesture/test.txt',
                               samplingTime=1.,
                               sampleLength=1500)

gen_train = DataLoader(dataset=trainingSet, batch_size=72, shuffle=True, num_workers=1)
gen_test = DataLoader(dataset=testingSet, batch_size=72, shuffle=True, num_workers=1)
...
for k, v in net.name_param():                                                           
    for i in range(4):
        if k == 'LIF_layers.{:d}.base_layer.weight'.format(i):
            v = torch.FloatTensor(np.load('Trained/fc{:d}.npy'.format(i))).to(device)
            print('v is set for fc{:d}.npy'.format(i), v.shape,v[0])
##Resume if necessary
