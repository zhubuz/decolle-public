...
def write_stats(epoch, test_acc, test_loss,spike, writer):
    for i, [l, a, s] in enumerate(zip(test_loss, test_acc, spike)):
        writer.add_scalar('/test_loss/layer{}'.format(i), l, epoch)
        writer.add_scalar('/test_acc/layer{}'.format(i), a, epoch)
        writer.add_scalar('spikes/layer{}'.format(i), torch.sum(s), epoch)
