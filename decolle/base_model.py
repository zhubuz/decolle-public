...
class LIFLayer(nn.Module):
    ...
    def forward(self, Sin_t):
        ...
        #print('P R', self.base_layer(P).shape, R.shape)
...
class DECOLLEBase(nn.Module):
    ...
    def decolle_linear(self, inF, outF, alpha=.9, alpharp=.65, wrp=1.0, beta=.85, deltat=1000, do_detach=True,lif_layer_type = LIFLayer,out_channels=0, lc_ampl=.5, is_output_layer=False):
...
class DECOLLELoss(object):
    ...
    def __call__(self, s, r, u, target, mask=1, sum_=True):
        ...
        for i,loss_layer in enumerate(self.loss_fn):
            ...
            # print("out,target",i, r[i].shape, target.shape)
